...
export default function ShareButtons({ url, title }: ShareButtonsProps) {
  ...
  const fullUrl = `https://www.ismybabyalright.com${url}`;
  ...
  return (
    <div className="flex items-center gap-2 flex-wrap">
      <span className="text-xs text-muted font-semibold mr-1">Share:</span>

      <button
        onClick={handleCopy}
        className="inline-flex items-center gap-1 rounded-full border border-[#E8E2D9] bg-white px-3 py-1.5 text-xs text-muted hover:border-primary hover:text-primary transition-colors"
      ...
      <a
        ...
        className="inline-flex items-center gap-1 rounded-full border border-[#E8E2D9] bg-white px-3 py-1.5 text-xs text-muted hover:border-primary hover:text-primary transition-colors no-underline"
