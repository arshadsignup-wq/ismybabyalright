...
export async function generateMetadata({
  searchParams,
}: ConcernsPageProps): Promise<Metadata> {
  const params = await searchParams;
  const page = parseInt(params.page || "1", 10) || 1;
  const canonical = page > 1 ? `/concerns?page=${page}` : "/concerns";

  return {
    title: `${concernCount} Common Baby Concerns - Evidence-Based Answers`,
    description: `Evidence-based answers to over ${allConcerns.length} common baby concerns — from rashes and poop to sleep, feeding, and milestones. Know when it's normal and when to call the doctor.`,
    alternates: {
      canonical,
    },
    openGraph: {
    ...
    <div>
      ...
      <div className="mx-auto max-w-5xl px-4 py-8 sm:px-6">
        ...
        <div className="mb-8 text-center">
          ...
          <p className="text-base text-muted leading-relaxed mb-6">Our concern guides cover over 2,500 common baby health questions with age-specific information, action tiers (normal, mention at next visit, call now), and source citations from the AAP, CDC, and WHO.</p>
